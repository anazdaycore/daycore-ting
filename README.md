...
`docs/specs/frontend-manifest.md`「前端的多语言形态」那四条：

1. **文案是数据不是代码** —— 语言包在 `public/locales/*.json`，**运行时 fetch**。
   构建产物旁边丢一个 `de-DE.json`（后端 `LOCALES_DIR` 里也丢一份）就多一门语言，
   不用重新编译。`import` 进来会把它内联进 bundle，那就又变回编译期字典了。
2. **可选语言从后端读** —— `POST /api/version` 的 `locales.available`。运维往
   `LOCALES_DIR` 丢文件就会多一门，汀 写死自己以为存在的那几种就会漏掉它。
3. **回退链和后端一致** —— 精确 → 同基础语言（`zh-TW` 先找 `zh-CN`）→ 默认 → 任意。
   最后一步看着像错的：半翻译的界面显示读者可能不认识的语言，好过显示空白 ——
   缺翻译是**内容**缺口，空屏读起来是**渲染故障**，人会绕过前者、给后者报 bug。
4. **缺 key 就显示 key 本身** —— 不是空字符串（看起来像布局 bug），不是现编一个
   （看起来像翻译错了）。屏幕上一个 key 明确地说「这条还没人写」，并且说是哪条。

**两份目录**，分工是承重的：`bootstrapCatalog()` 用汀 自己带的包，管**还没连上
后端**的那两屏（首次设置、连不上）；真正的目录用**部署报上来**的语言列表，管其余
全部。合成一份就只能二选一 —— 要么设置页不可翻译，要么语言列表写死。

`src/locales.test.ts` 是这一层的闸门，进 `npm run build`：包之间 key 对齐、没有
空值、占位符两边一致、源码要的 key 都有、包里的 key 都有人要、**应用源码里没有
CJK 字面量**。七种破坏方式全部反验过。
